...
// Udyo360 Images — hardened container image catalog (public, no auth)
...
// Udyo360 Images — custom image builder & requests
...
// Udyo360 Charts — hardened chart catalog (public, no auth)
...
// Udyo360 Libraries — hardened language package catalog (public, no auth)
...
// Udyo360 Runtime (formerly Udyo360Runtime) — supply-chain visibility (public, no auth)
// Canonical route is /api/runtime/*; backend keeps /api/cleansight/* as a fallback.
export const getUdyo360RuntimeStats  = ()   => request('/api/runtime/stats')
export const getUdyo360Runtime = ({ search = '', category = '', type = '', sort = '', page = 1, pageSize } = {}) => {
  const q = new URLSearchParams()
  if (search)   q.set('search', search)
  if (category) q.set('category', category)
...
export const getUdyo360RuntimeDetail = (id) => request(`/api/runtime/${encodeURIComponent(id)}`)

// Udyo360 Assistant — site-wide Q&A chat (public)
